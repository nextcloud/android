...
class OCFileListFragmentStaticServerIT : AbstractIT() {
    ...
    @get:Rule
    val storagePermissionRule = GrantStoragePermissionRule.grant()
        ...
        launchActivity<TestActivity>().use { scenario ->
            scenario.onActivity { sut ->
                onIdleSync {
                    ...
                    Assert.assertFalse(fragment.adapter.shouldShowHeader())

                    testFolder.richWorkspace = " "
                    sut.storageManager.saveFile(testFolder)
                    fragment.adapter.swapDirectory(user, testFolder, sut.storageManager, false, "")
                    Assert.assertFalse(fragment.adapter.shouldShowHeader())
                    ...
                    sut.storageManager.saveFile(testFolder)
                    fragment.adapter.setCurrentDirectory(testFolder)
                    Assert.assertTrue(fragment.adapter.shouldShowHeader())
                }
            }
        }
    }
