...
class LegacyMigrationHelper(private val clock: Clock) {
    fun onUpgrade(db: SupportSQLiteDatabase, oldVersion: Int, newVersion: Int) {
        ...
        if (oldVersion < 25 && newVersion >= 25) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME +
                ...
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME +
        ...
        if (oldVersion < 27 && newVersion >= 27) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME +
        ...
        if (oldVersion < 28 && newVersion >= 28) {
            ...
            try {
                if (!checkIfColumnExists(
                        db, ProviderMeta.ProviderTableMeta.FILESYSTEM_TABLE_NAME,
        ...
        if (oldVersion < 30 && newVersion >= 30) {
            ...
            try {
                if (!checkIfColumnExists(
                        db, ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME,
                ...
                if (oldVersion > ARBITRARY_DATA_TABLE_INTRODUCTION_VERSION) {
                    if (!checkIfColumnExists(
                            db, ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME,
        ...
        if (oldVersion < 32 && newVersion >= 32) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.OCSHARES_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.OCSHARES_IS_PASSWORD_PROTECTED + " INTEGER "
                ) // boolean
                upgraded = true
                db.setTransactionSuccessful()
            } finally {
        ...
        if (oldVersion < 34 && newVersion >= 34) {
            ...
            try {
                if (!checkIfColumnExists(
                        db, ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_TABLE_NAME,
                        ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_REDIRECT
                    )
                ) {
                    db.execSQL(
                        ALTER_TABLE + ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_TABLE_NAME +
                            ADD_COLUMN + ProviderMeta.ProviderTableMeta.EXTERNAL_LINKS_REDIRECT + " INTEGER "
                    ) // boolean
                }
                upgraded = true
                db.setTransactionSuccessful()
        ...
        if (oldVersion < 38 && newVersion >= 38) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.CAPABILITIES_RICHDOCUMENT + " INTEGER "
                ) // boolean
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.CAPABILITIES_RICHDOCUMENT_MIMETYPE_LIST + " TEXT "
                ) // string
                upgraded = true
                db.setTransactionSuccessful()
            } finally {
        ...
        if (oldVersion < 39 && newVersion >= 39) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.CAPABILITIES_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.CAPABILITIES_RICHDOCUMENT_DIRECT_EDITING + " INTEGER "
                ) // bool
                upgraded = true
                db.setTransactionSuccessful()
            } finally {
        ...
        if (oldVersion < 50 && newVersion >= 50) {
            ...
            try {
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.SYNCED_FOLDER_ENABLED_TIMESTAMP_MS + " INTEGER "
                )
                db.execSQL(
                    "UPDATE " + ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME + " SET " +
        ...
        if (oldVersion < 54 && newVersion >= 54) {
            Log_OC.i(
                TAG, "Entering in the #54 add synced.existing," +
            ...
            try {
                ...
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.SYNCED_FOLDER_EXISTING + " INTEGER "
                ) // boolean

                // Rename uploads.force_overwrite to uploads.name_collision_policy
                val tmpTableName = ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME + "_old"
                db.execSQL(ALTER_TABLE + ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME + " RENAME TO " + tmpTableName)
                createUploadsTable(db)
                db.execSQL(
                    "INSERT INTO " + ProviderMeta.ProviderTableMeta.UPLOADS_TABLE_NAME + " (" +
        ...
        if (oldVersion < 55 && newVersion >= 55) {
            ...
            try {
                ...
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.SYNCED_FOLDERS_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.SYNCED_FOLDER_NAME_COLLISION_POLICY + " INTEGER "
                ) // integer
        ...
        if (oldVersion < 56 && newVersion >= 56) {
            ...
            try {
                ...
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.FILE_PATH_DECRYPTED + " TEXT "
                ) // strin
                upgraded = true
                db.setTransactionSuccessful()
            } finally {
        ...
        if (oldVersion < 63 && newVersion >= 63) {
            ...
            try {
                ...
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME +
                        ADD_COLUMN + ProviderMeta.ProviderTableMeta.FILE_LOCKED + " INTEGER "
                ) // boolean
                db.execSQL(
                    ALTER_TABLE + ProviderMeta.ProviderTableMeta.FILE_TABLE_NAME +
    ...
    private fun createUploadsTable(db: SupportSQLiteDatabase) {
        db.execSQL(
    ...
    */
    ...
    private fun checkIfColumnExists(database: SupportSQLiteDatabase, table: String, column: String): Boolean {
        val cursor = database.query("SELECT * FROM $table LIMIT 0")
        val exists = cursor.getColumnIndex(column) != -1
        cursor.close()
        return exists
    }

    companion object {
        private val TAG = LegacyMigrationHelper::class.java.simpleName
        const val ARBITRARY_DATA_TABLE_INTRODUCTION_VERSION = 20
        private const val ALTER_TABLE = "ALTER TABLE "
        private const val ADD_COLUMN = " ADD COLUMN "
        private const val INTEGER = " INTEGER, "
        private const val TEXT = " TEXT, "
        private const val UPGRADE_VERSION_MSG = "OUT of the ADD in onUpgrade; oldVersion == %d, newVersion == %d"
    }
}