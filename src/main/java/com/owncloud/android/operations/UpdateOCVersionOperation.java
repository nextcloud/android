...
public class UpdateOCVersionOperation extends RemoteOperation<Void> {
    ...
    protected RemoteOperationResult<Void> run(OwnCloudClient client) {
        ...
        String statUrl = accountManager.getUserData(account, Constants.KEY_OC_BASE_URL);
        statUrl += STATUS_PATH;
        RemoteOperationResult<Void> result = null;
                ...
                if (response != null) {
                    ...
                    if (json.has("version")) {
                        ...
                        if (mOwnCloudVersion.isVersionValid()) {
                            accountManager.setUserData(account, Constants.KEY_OC_VERSION, mOwnCloudVersion.getVersion());
