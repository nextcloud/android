...
deleteOldComments() {
    ...
    oldComments=$(curl > /dev/null 2>&1 --header "authorization: Bearer $GITHUB_TOKEN" -X GET https://api.github.com/repos/nextcloud/android/issues/$PR/comments | jq --arg TYPE $BRANCH_TYPE '.[] | (.id |tostring) + "|" + (.user.login | test("nextcloud-android-bot") | tostring) + "|" + (.body | test([$TYPE]) | tostring)'| grep "true|true" | tr -d "\"" | cut -f1 -d"|")
    count=$(echo $oldComments | grep true | wc -l)
    echo "Found $count old comments"

    echo $oldComments | while read comment ; do
        echo "Deleting comment: $comment"
        curl > /dev/null 2>&1 --header "authorization: Bearer $GITHUB_TOKEN" -X DELETE https://api.github.com/repos/nextcloud/android/issues/comments/$comment
    done
}
...
upload() {
    ...
    curl > /dev/null 2>&1 --header "authorization: Bearer $GITHUB_TOKEN" -X POST https://api.github.com/repos/nextcloud/android/issues/$PR/comments \
...
if [ -e $FOLDER ]; then
    ...
    curl > /dev/null 2>&1 --header "authorization: Bearer $GITHUB_TOKEN" \
